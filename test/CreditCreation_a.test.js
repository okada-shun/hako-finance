...
contract('CreditCreation_a', ([alice, bob, carol, dave, ...accounts]) => {
  ...
  describe('creditCreationByMember', () => {
    ...
    it('should not allow non-hako member accounts to create creditToHako', async function () {
      await this.hako.transfer(bob, 300, {from: alice});
      await utils.shouldThrow(this.hako.creditCreationByMember(100, {from: bob}));
    });
    ...
    it('should not allow hako members who have debtToHako to create creditToHako', async function () {
      ...
      await utils.shouldThrow(this.hako.creditCreationByMember(100, {from: carol}));
    ...
    it('should not allow hako members to register more borrowValue than upperLimit', async function () {
      ...
      await utils.shouldThrow(this.hako.creditCreationByMember(700, {from: bob}));
    });
    ...
    it('should not allow hako members whose netAssets is minus to create creditToHako', async function () {
      ...
      await this.hako.joinHako(200, {from: carol});
      await this.hako.joinHako(100, {from: dave});
      await this.hako.registerBorrowValueDuration(200, 60, {from: bob});
    ...
    it('should not allow hako members to create more creditToHako than his netAssets', async function () {
      ...
      await utils.shouldThrow(this.hako.creditCreationByMember(500, {from: bob}));
      await this.hako.registerBorrowValueDuration(200, 60, {from: bob});
      await this.hako.lendCredit(bob, 200, 60, {from: carol});
      ...
      await this.hako.creditCreationByMember(500, {from: bob});
    });

    it('should emit a CreditCreationByMember event', async function () {
      await this.hako.transfer(bob, 300, {from: alice});
      await this.hako.joinHako(300, {from: bob});
      const {logs} = 
        await this.hako.creditCreationByMember(100, {from: bob});
      const event = await expectEvent.inLogs(logs, 'CreditCreationByMember');
