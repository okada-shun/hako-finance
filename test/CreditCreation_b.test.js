...
contract('CreditCreation_b', ([alice, bob, carol, dave, ...accounts]) => {
  ...
  describe('reduceDebt', () => {
    ...
    it('should allow a hako member to reduce his creditToHako and debtToHako', async function () {
      ...
      await this.hako.reduceDebt(50, {from: bob});
    ...
    it('should allow multiple hako members to reduce their creditToHako and debtToHako', async function () {
      await this.hako.transfer(bob, 300, {from: alice});
      ...
      await this.hako.reduceDebt(50, {from: bob});
      await this.hako.reduceDebt(20, {from: carol});
      await this.hako.reduceDebt(10, {from: dave});
      //creditToHako of bob is 350 (= 300 + 100 - 50).
      //debtToHako of bob is 50 (= 100 - 50).
    ...
    it('should allow hako members to return their debtToHako to hako', async function () {
      ...
      await this.hako.reduceDebt(50, {from: carol});
    ...
    it('should not allow hako members to reduce more creditToHako or debtToHako than they have', async function () {
      await this.hako.transfer(bob, 300, {from: alice});
      await this.hako.transfer(carol, 200, {from: alice});
      await this.hako.transfer(dave, 100, {from: alice});
      await this.hako.joinHako(300, {from: bob});
      ...
      await utils.shouldThrow(this.hako.reduceDebt(200, {from: bob}));
      await this.hako.reduceDebt(100, {from: bob});
      await this.hako.createCredit(300, {from: bob});
      await this.hako.transferCredit(carol, 600, {from: bob});
      //bob has 0 creditToHako, 300 debtToHako.
      await utils.shouldThrow(this.hako.reduceDebt(200, {from: bob}));
    });

    it('should emit a ReduceDebt event', async function () {
      await this.hako.transfer(bob, 300, {from: alice});
      await this.hako.joinHako(300, {from: bob});
      await this.hako.createCredit(100, {from: bob});
      const {logs} = 
        await this.hako.reduceDebt(50, {from: bob});
      const event = await expectEvent.inLogs(logs, 'ReduceDebt');
